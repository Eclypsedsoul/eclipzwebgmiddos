...
bot = telebot.TeleBot('7229595341:AAFd2yOJNtnE1RdyHEp9WhoAvYno7ZZIHKU')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 20:
                response = "You Are On Cooldown ❌. Please Wait 20 second Before Running The /attack Command Again."
                bot.reply_to(message, response)
                return
...
def welcome_start(message):
    ...
    response = f'''👋🏻Welcome to The Bot, user ♥️
      🤖Feel Free to Use. 
 for more information contact:- https://t.me/eclypsedsoul'''
    bot.reply_to(message, response)
